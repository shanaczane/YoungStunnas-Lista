...
function getMemberAvatar(name, members) {
  ...
  return (
    members.find((m) => m.display_name?.toLowerCase() === name.toLowerCase())
      ?.avatar_url || null
  );
}
...
export default function SpacesScreen({
  ...
  async function fetchSpacePreviews(fetchedSpaces, membersMap) {
    ...
    for (const spaceId of spaceIds) {
      ...
      if (!task) {
        previews[spaceId] = "No activity yet";
        continue;
      }
  ...
  async function handleCreateSpace({ name, description, color }) {
    const { data, error } = await supabase
      .from("spaces")
      .insert({ name, description, color, owner_id: session.user.id })
      .select()
      .single();
    if (!error && data) {
      const myAvatar =
  ...
  async function handleDeleteFromList(spaceId) {
    ...
    if (sessionStorage.getItem(ACTIVE_SPACE_KEY) === spaceId)
      sessionStorage.removeItem(ACTIVE_SPACE_KEY);
    ...
    return (
      <SpaceBoard
        ...
        onSpaceUpdated={(updates) =>
          setSpaces((prev) =>
            prev.map((s) =>
              s.id === activeSpace.id ? { ...s, ...updates } : s,
            ),
          )
        }
      />
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-app-bg">
      <ScreenHeader>
        <div className="min-w-0">
          <h1 className="text-slate-900 font-bold text-[22px] leading-tight">
            Spaces
          </h1>
        ...
        {invites.length > 0 && (
          <div className="flex flex-col gap-2.5 mb-4">
            ...
            {invites.map((invite) => (
              <div
                key={invite.id}
                className="rounded-2xl bg-accent-pale border border-accent-light/40 px-3.5 py-3.5 flex items-center gap-3"
              >
            ...
            {sorted.map((space) => {
              ...
              return (
                <div
                  ...
                  style={{
                    backgroundColor: spaceColor + "18",
                    boxShadow: `0 2px 12px ${spaceColor}22`,
                  }}
                >
                  <button
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-slate-900 font-bold text-[15px] leading-tight truncate flex-1">
                          {space.name}
                        </p>
                        {ts && (
                      ...
                      <p className="text-slate-500 text-[13px] mt-1 truncate">
                        {(() => {
                          const text =
                            preview ?? (space.description || "No activity yet");
...
function SpaceBoard({
  ...
  const [viewMode, setViewMode] = useState(
    () => sessionStorage.getItem(`lista_viewmode_${space.id}`) || "list",
  );
  const [inProgressIds, setInProgressIds] = useState(() => {
    try {
      return new Set(
        JSON.parse(localStorage.getItem(`lista_kanban_${space.id}`) || "[]"),
      );
    } catch {
      return new Set();
    }
  ...
  const [moveLog, setMoveLog] = useState(() => {
    try {
      return JSON.parse(
        localStorage.getItem(`lista_moves_${space.id}`) || "[]",
      );
    } catch {
      return [];
    }
  ...
  function handleDrop(task, toColId) {
    const fromColId = task.is_complete
      ? "done"
      : inProgressIds.has(task.id)
        ? "inprogress"
        : "todo";
  ...
  return (
    <div
      className={`flex flex-col h-screen overflow-hidden ${!spaceData.color ? "bg-app-bg" : ""}`}
      style={boardBg ? { backgroundColor: boardBg } : undefined}
    >
      <ScreenHeader className="px-4 pt-5 pb-3">
        <button
          onClick={onBack}
          className="flex items-center gap-1 text-accent-deep text-sm font-medium mb-3 active:opacity-70"
        >
          <svg
            width="16"
            height="16"
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-slate-900 font-bold text-[20px] leading-tight truncate">
                {spaceData.name}
              </h1>
              ...
              <div key={member} className="space-y-2">
                <div className="flex items-center gap-2 px-1 pt-1">
                  ...
                  >
                    <MemberAvatar
                      name={member}
                      avatarUrl={getMemberAvatar(member, members)}
                      sizePx={24}
                      fontSize={10}
                    />
                  </button>
      ...
      <div className="fixed bottom-16 left-0 right-0 z-10 px-4 pb-3 pt-2 bg-app-bg/96 backdrop-blur-md flex flex-col gap-2.5">
        {parseCard && (
          <div
            className="rounded-2xl p-4 card-elevated-lg"
            style={{
              backgroundColor: spaceData.color
                ? spaceData.color + "28"
                : "var(--color-card-bg)",
            }}
          >
            ...
            <div className="flex gap-2 mt-3">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl border border-divider text-slate-500 text-sm font-medium"
              >
                Cancel
              </button>
              <button
        ...
        <div
          className="flex items-center gap-2 border border-divider rounded-2xl px-4 py-3 shadow-sm"
          style={{
            backgroundColor: spaceData.color
              ? spaceData.color + "28"
              : "var(--color-card-bg)",
        ...
        <div className="relative flex flex-col items-center">
          {effectiveAssignee && (
            ...
            >
              <MemberAvatar
                ...
                avatarUrl={getMemberAvatar(effectiveAssignee, members)}
                sizePx={32}
                fontSize={11}
              />
            </button>
          ...
          {creatorName && !sameAsCreator && (
            ...
            >
              <MemberAvatar
                name={creatorName}
                avatarUrl={getMemberAvatar(creatorName, members)}
                sizePx={24}
                fontSize={10}
              />
            </button>
...
function SpaceTaskModal({
  ...
  if (modification) {
    const detail = modification.changes
      .map((c) => (typeof c === "object" ? `${c.field} → "${c.to}"` : c))
      .join(" · ");
    history.push({
      label: "Edited",
      date: new Date(modification.at),
      icon: "✎",
      color: "#8B5CF6",
      person: modification.by,
      detail,
    });
  }
  if (task.is_complete) {
    ...
    history.push({
      ...
      icon: "✓",
      color: "#34d399",
      person: sanAssignee || creatorName,
    });
  }
...
function SpaceSettingsModal({
  ...
  useEffect(() => {
    ...
    supabase
      .from("space_invites")
      .select("id, invited_email, user_code, invited_by_name")
      .eq("space_id", space.id)
      .eq("status", "pending")
      .then(({ data }) => {
        if (data) setSentInvites(data);
      });
  }, [space.id]);
  ...
  async function handleAddMember() {
    ...
    if (addMode === "email") {
      ...
      if (!error) {
        setMemberInput("");
        setMemberSuccess(
          `Invite sent to ${val}! They'll see it when they open Lista.`,
        );
      ...
      const clean = val
        ...
        .toUpperCase();
      if (clean.length < 6) {
        setMemberError("Invalid User ID — format: LISTA-XXXXXXXX");
        setAddingMember(false);
        return;
      }
      const senderName =
      ...
      if (!error) {
        ...
        setMemberError(
          "Could not send invite. Make sure the space_invites table exists in Supabase.",
        );
      }
    }
    setAddingMember(false);
  ...
  function handleCopyLink() {
    const base =
      import.meta.env.VITE_APP_URL || "https://lista-orpin.vercel.app";
      ...
      <div className="w-full bg-card-bg rounded-t-3xl max-h-[92vh] flex flex-col shadow-2xl">
        ...
        <div className="overflow-y-auto flex-1 px-6 pb-4">
          ...
          <div className="mb-5 bg-accent-pale border border-accent-light/30 rounded-2xl px-4 py-3.5">
            <p className="text-accent-deep text-xs font-bold uppercase tracking-wide mb-2">
              Share Space
            </p>
            <p className="text-slate-500 text-xs mb-3 leading-snug">
              Copy this link and send it to anyone you want to invite to this
              space.
            </p>
            <button
              ...
              className={`w-full py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${
                linkCopied
                  ...
                  : "bg-accent-deep text-white active:bg-accent-mid"
              }`}
            >
              {linkCopied ? (
                <>
          ...
          <div className="mb-5">
            ...
            <div className="space-y-2 mb-3">
                ...
                <div key={m.user_id}>
                  ...
                  {confirmRemoveId === m.user_id && (
                    <div className="mt-1 mx-1 bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2.5 flex items-center gap-2">
                      <p className="flex-1 text-red-500 text-xs font-medium">
                        Remove {m.display_name}?
                      </p>
                      <button
                        onClick={() => setConfirmRemoveId(null)}
                        className="text-slate-400 text-xs px-2 py-1 rounded-lg border border-slate-200"
                      >
                        Cancel
                      </button>
                      <button
                        onClick={() => handleRemoveMember(m.user_id)}
                        className="text-white text-xs px-2.5 py-1 rounded-lg bg-red-500 font-semibold"
                      >
                        Remove
                      </button>
                    </div>
              ...
              {sentInvites.map((inv) => (
                <div
                  key={inv.id}
                  className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2.5"
                >
                  <div className="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center text-amber-500 text-sm font-bold flex-shrink-0">
                    ?
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-700 text-sm font-medium truncate">
                      {inv.invited_email || `LISTA-${inv.user_code}`}
                    </p>
            ...
            {addMode === "userid" && (
              <p className="text-slate-400 text-[11px] mb-2 leading-snug">
                Ask your teammate to copy their User ID from their Lista profile
                (tap avatar → Profile).
              </p>
            )}
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  addMode === "email" ? "teammate@gmail.com" : "LISTA-XXXXXXXX"
                }
            ...
            {memberError && (
              <p className="text-red-500 text-xs mt-1.5">{memberError}</p>
            )}
            {memberSuccess && (
              <p className="text-green-600 text-xs mt-1.5">{memberSuccess}</p>
            )}
          </div>
...
function ActivityDrawer({
  ...
  const iconColor = (ev) => {
    ...
    if (ev.type === "moved")
      return KANBAN_COLS.find((c) => c.id === ev.toId)?.color || "#a78bfa";
    return spaceColor;
  };
  const iconGlyph = (ev) => {
              ...
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                  activeTab === tab.id
                    ...
                    : "text-slate-400 border border-divider"
                }`}
              >
                {tab.label}
                {count > 0 && (
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 px-3 py-3 border-b border-black/6">
              ...
              <span className="ml-auto text-[10px] font-bold bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded-full">
                {cols[col.id].length}
              </span>
            </div>
            <div
              className={`flex flex-col gap-2 flex-1 min-h-[88px] p-2.5 transition-all duration-200 ${isOver ? "ring-2 ring-dashed" : ""}`}
            ...
            >
              ...
              {cols[col.id].length === 0 && (
                ...
                >
                  <p
                    ...
                    style={isOver ? { color: col.color } : { color: "#cbd5e1" }}
                  >
              ...
              {isOver && cols[col.id].length > 0 && (
                <div
                  className="h-12 border-2 border-dashed rounded-2xl flex items-center justify-center transition-all duration-150"
                  style={{
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <span
          className="min-w-0 truncate text-[9px] font-semibold px-1.5 py-0.5 rounded-full"
          style={{ backgroundColor: colors.bg, color: colors.text }}
        >
          {task.category}
        </span>
      ...
      <p
        className={`text-sm font-semibold leading-tight mb-1.5 break-words ${task.is_complete ? "line-through text-slate-300" : "text-slate-800"}`}
      >
        {task.task_name}
      </p>
      {modification && (
      ...
      <div className="md:hidden mb-2 pt-2 border-t border-black/8">
        <p className="text-[9px] font-bold uppercase tracking-wide text-slate-300 mb-1.5">
          Move to
        </p>
...
function MemberView({
  ...
  const memberData = members.map((m) => {
    ...
    const assigned = tasks.filter((t) => {
      const raw =
        t.assignee && t.assignee !== "null" && t.assignee !== "undefined"
          ? t.assignee
          : null;
      const effective =
        raw || members.find((mx) => mx.user_id === t.user_id)?.display_name;
    ...
    const inProgress = assigned.filter(
      (t) => inProgressIds.has(t.id) && !t.is_complete,
    ).length;
    const overdue = assigned.filter(
      (t) => t.due_date && !t.is_complete && new Date(t.due_date) < new Date(),
    ).length;
    const lastActive = assigned.reduce((latest, t) => {
      const d = new Date(
        modifications[t.id]?.at || t.updated_at || t.created_at,
      );
...
function MemberTaskCard({
  ...
  return (
    ...
    >
      <div
        className="w-1 flex-shrink-0"
        style={{ backgroundColor: modification ? "#8B5CF6" : colors.border }}
      />
      <div className="flex-1 px-3 py-2.5 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <span
            className="flex-shrink-0 px-2 py-0.5 rounded-full text-[9px] font-bold text-white"
        ...
        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
          ...
          {task.due_date && (
            <span
              className={`text-[9px] font-medium ${isOverdue ? "text-red-400" : "text-slate-400"}`}
            >
              {isOverdue ? "⚠ " : ""}
              {formatDueDate(task.due_date)}
            </span>
          ...
          {modification && (
            <span className="text-[9px] font-bold text-purple-500 bg-purple-500/10 px-1.5 py-0.5 rounded-full">
              Modified
            </span>
          )}
        </div>
        ...
        <table className="w-full table-fixed border-collapse">
          <thead>
            <tr style={{ backgroundColor: headerBg }}>
              ...
              <th className="px-1 py-2.5 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wide border-b border-black/8">
                Task
              </th>
              <th className="w-[78px] px-1 py-2.5 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wide border-b border-black/8">
                Status
              </th>
              <th className="w-[58px] px-2 py-2.5 text-right text-[10px] font-bold text-slate-400 uppercase tracking-wide border-b border-black/8">
                Due
              </th>
            </tr>
          </thead>
          <tbody>
            {tasks.length === 0 ? (
              <tr>
                <td
                  colSpan={4}
                  className="py-16 text-center text-slate-300 text-sm"
                >
                  No tasks
                </td>
              </tr>
          ...
          <thead>
            <tr style={{ backgroundColor: headerBg }}>
              {[
                ...
                <th
                  key={i}
                  className={`py-2.5 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wide border-b border-black/8 ${i === 0 ? "w-10 px-4" : "px-3"}`}
                >
                  {h}
                </th>
              ))}
        ...
        >
          <div className="flex items-start gap-3 px-3.5 py-3.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-start justify-between gap-2">
                <p
                  className={`min-w-0 text-[15px] font-bold leading-snug ${task.is_complete ? "line-through text-slate-300" : "text-slate-900"}`}
                >
                  {task.task_name}
                </p>
                <div
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                ...
                {task.due_date && (
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold ${isOverdue ? "bg-red-50 text-red-500" : "bg-slate-50 text-slate-500"}`}
                  >
                    {formatDueDate(task.due_date)}
                  </span>
                )}
              ...
              <div className="mt-3 flex items-center justify-between gap-2 border-t border-black/8 pt-2.5">
                ...
                <div className="text-right">
                  <p className="text-[9px] font-bold uppercase tracking-wide text-slate-300">
                    Due
                  </p>
                  <p
                    className={`text-[11px] font-semibold ${isOverdue ? "text-red-400" : "text-slate-500"}`}
                  >
                    {task.due_date
...
function MobilePersonCell({ label, name, members, onMemberClick }) {
  return (
    <div className="min-w-0 rounded-xl bg-white/60 border border-black/6 px-2.5 py-2">
      <p className="text-[9px] font-bold uppercase tracking-wide text-slate-300 mb-1">
        {label}
      </p>
      {name ? (
        <button
        ...
        >
          <MemberAvatar
            name={name}
            avatarUrl={getMemberAvatar(name, members)}
            sizePx={22}
            fontSize={9}
          />
  ...
  return (
    ...
    >
      ...
      <td className="py-3 px-3 max-w-[180px]">
        <p
          className={`text-sm font-medium leading-tight truncate ${task.is_complete ? "line-through text-slate-300" : "text-slate-800"}`}
        >
          {task.task_name}
        </p>
      ...
      <td className="py-3 px-3 w-28">
        {assignee && (
          ...
          >
            ...
            <span className="text-xs text-slate-500 truncate max-w-[56px] group-hover/av:text-accent-deep transition-colors">
              {assignee}
            </span>
          </button>
        )}
      ...
      <td className="py-3 px-3 w-24">
        {task.due_date ? (
          <span
            className={`text-xs font-medium whitespace-nowrap ${isOverdue ? "text-red-400" : "text-slate-400"}`}
          >
            {new Date(task.due_date).toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
...
function SpaceDetailsModal({ tasks, members, spaceColor, onClose }) {
  ...
  const catStats = ["Work", "Personal", "School", "Errands", "Health"]
    .map((cat) => {
      const ct = tasks.filter((t) => t.category === cat);
      return {
        cat,
        total: ct.length,
        done: ct.filter((t) => t.is_complete).length,
      };
    })
    .filter((s) => s.total > 0);
  ...
  const memberStats = members.map((m) => {
    ...
    return {
      ...
      done: assigned.filter((t) => t.is_complete).length,
    };
  });
      ...
      <div className="w-full bg-card-bg rounded-t-3xl max-h-[80vh] flex flex-col shadow-2xl">
        ...
        <div className="overflow-y-auto flex-1 px-6 pb-8 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Total", value: total, color: spaceColor },
          ...
          <div>
            ...
            <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
              <div
                ...
                style={{ width: `${rate}%`, backgroundColor: spaceColor }}
              />
            </div>
          </div>
          {/* By category */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="h-1 rounded-full bg-slate-100 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${pct}%`,
                            backgroundColor: memberColor(s.name),
                          }}
                        />
                      </div>
                    </div>
